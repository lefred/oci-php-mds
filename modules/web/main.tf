...
locals {
  ...
  create_app_db    = "~/create_app_db.sh"
...
data "template_file" "create_app_db" {
  template = file("${path.module}/scripts/create_app_db.sh")
...
resource "oci_core_instance" "Application" {
 ...
 provisioner "file" {
    content     = data.template_file.create_app_db[count.index].rendered
    destination = local.create_app_db
   ...
   provisioner "remote-exec" {
    ...
    inline = [
       ...
       "chmod +x ${local.create_app_db}",
       "sudo ${local.create_app_db}"
    ]
