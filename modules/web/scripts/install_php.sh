...
then
  ...
  phpver=${php_version}
  phpversion=$(echo $phpver | tr -d '.')
  yum-config-manager --enable remi-php$phpversion
  yum -y install php php-cli php-mysqlnd php-zip php-gd php-mcrypt php-mbstring php-xml php-json php-fpm php-pecl-mysql-xdevapi
  yum -y install certbot mod_ssl
  yum -y install git
fi

echo "MySQL Shell & PHP ${php_version} successfully installed !"
